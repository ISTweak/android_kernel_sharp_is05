...
#endif
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
static struct perf_lock suspend_perf_lock;
...
#if defined(CONFIG_PERFLOCK_SCREEN_POLICY) || \
    defined(CONFIG_SHSYS_CUST_PERFLOCK)
...
#else
#define BOOT_LOCK_TIMEOUT	(120 * HZ)
#endif
#else
#define BOOT_LOCK_TIMEOUT	(60 * HZ)
#endif

#define BOOT_WAKE_LOCK_NAME "BOOT_WAKE_LOCK"
...
void __init perflock_init(struct perflock_platform_data *pdata)
{
	...
	if(bootmode == SH_BOOT_NORMAL){
		wake_lock_init(&boot_wake_lock, WAKE_LOCK_SUSPEND, BOOT_WAKE_LOCK_NAME);
...
#ifdef CONFIG_SHSYS_CUST_PERFLOCK
	perf_lock_init(&suspend_perf_lock, PERF_LOCK_HIGHEST, "suspend");
	perf_lock_init(&enter_state_perf_lock, PERF_LOCK_368MHz, "enter_state");
#endif
