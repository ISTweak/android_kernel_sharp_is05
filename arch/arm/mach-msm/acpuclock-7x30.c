...
static struct clkctl_acpu_speed acpu_freq_tbl[] = {
	...
	{ 1, 1900800, PLL_2,    3, 0,  UINT_MAX, 1475, VDD_RAW(1475), &pll2_tbl[8]},
	{ 1, 2048000, PLL_2,    3, 0,  UINT_MAX, 1475, VDD_RAW(1475), &pll2_tbl[9]},
	{ 0 }
...
unsigned int scsys_get_perflock_freq(unsigned int level)
{
	if(level >= ARRAY_SIZE(cpufreq_tbl) - 1){
		pr_err("scsys_get_perflock_freq() level incorrect.\n");
		BUG();
	}

	return cpufreq_tbl[level].frequency;
}
#endif
