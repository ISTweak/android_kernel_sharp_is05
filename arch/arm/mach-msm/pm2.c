...
#ifdef CONFIG_SDIO_CUST_SH
extern void msmsdcc_pm_idle_suspend(void);
extern void msmsdcc_pm_idle_resume(void);
#endif
...
enum {
	SH_PM_DEBUG_IDLE_CLK = 1U << 0,
	SH_PM_DEBUG_WAKEUP_REASON = 1U << 1,
};
...
#define SH_SLEEP_PROHIBIT_TIMEOUT (30 * HZ)
#else
#define SH_SLEEP_PROHIBIT_TIMEOUT (60 * HZ)
#endif

static int msm_pm_sleep_mode_original;
static int msm_pm_idle_sleep_mode_original;

#ifdef CONFIG_HAS_WAKELOCK
#define SLEEP_PROHIBIT_WAKE_LOCK_NAME "SLEEP_PROHIBIT_WAKE_LOCK"
static struct wake_lock sleep_prohibit_wake_lock;
#endif

static void do_expire_sleep_prohibit(struct work_struct *work)
{
	unsigned long flags;

	local_irq_save(flags);
	msm_pm_sleep_mode      = msm_pm_sleep_mode_original;
	msm_pm_idle_sleep_mode = msm_pm_idle_sleep_mode_original;
	local_irq_restore(flags);

#ifdef CONFIG_HAS_WAKELOCK
	wake_unlock(&sleep_prohibit_wake_lock);
	wake_lock_destroy(&sleep_prohibit_wake_lock);
#endif

	pr_info("end sleep prohibit\n");
}
static DECLARE_DELAYED_WORK(work_expire_sleep_prohibit, do_expire_sleep_prohibit);
#endif
...
#ifdef CONFIG_SHSYS_CUST_DEBUG
	if (!from_idle)
		pr_info( "msm_pm_power_collapse(): Enter suspend power collapse.\n" );
#endif
...
#ifdef CONFIG_SHSYS_CUST_DEBUG
	if (!from_idle)
		pr_info( "msm_pm_power_collapse(): Exit suspend power collapse. ret = %d \n", collapsed );
#endif
...
#ifdef CONFIG_SHSYS_CUST_DEBUG
	if (sh_pm_debug_mask & SH_PM_DEBUG_WAKEUP_REASON){
		pr_info( "msm_pm_power_collapse(): from %s : wakeup_reason=%u (rpc_prog=0x%x, rpc_proc=%u, gpioInfo=%u)\n",
...
void arch_idle(void)
{
	...
	if( audpreproc_get_wake_lock_status() == true ) {
		allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_STANDALONE] = false;
	}

#ifdef CONFIG_IDLECTRL_EMMC_CUST_SH
	if (mmc_get_idle_wake_lock_status() == true) {
		allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_STANDALONE] = false;
	}
...
#if defined(CONFIG_SHSYS_CUST) && defined(CONFIG_SDIO_CUST_SH)
		msmsdcc_pm_idle_suspend();
#endif

#ifdef CONFIG_SHSYS_CUST_DEBUG
		if (sh_pm_debug_mask & SH_PM_DEBUG_IDLE_CLK){
			pr_info( "arch_idle(): Execute clock_debug_print_enabled(). \n" );
...
#ifdef CONFIG_SHSYS_CUST_DEBUG
		if (sh_pm_debug_mask & SH_PM_DEBUG_IDLE_CLK)
			pr_info( "msm_pm_enter(): Execute clock_debug_print_enabled(). \n" );
#endif
		clock_debug_print_enabled();
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
		...
		} else if (!strcmp(cmd, "UIM-LCK")) {
			restart_reason = 0x77665585;
#endif
		} else {
 ...
 *****************************************************************************/
#ifdef CONFIG_SHSYS_CUST
static unsigned long sh_boot_get_bootmode(void)
{
...
static int __init msm_pm_init(void)
{
	...
	unsigned long bootmode = sh_boot_get_bootmode();
#endif
	...
	if (*pSleepSmemSleepDisabled) {
		...
		msm_pm_modes[MSM_PM_SLEEP_MODE_POWER_COLLAPSE_STANDALONE].supported = 0;
		msm_pm_modes[MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT].supported = 0;
	}

	if(bootmode == SH_BOOT_NORMAL){
		msm_pm_sleep_mode_original = msm_pm_sleep_mode;
		msm_pm_idle_sleep_mode_original = msm_pm_idle_sleep_mode;

		msm_pm_sleep_mode      = MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT;
		msm_pm_idle_sleep_mode = MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT;
#ifdef CONFIG_HAS_WAKELOCK
		wake_lock_init(&sleep_prohibit_wake_lock, WAKE_LOCK_SUSPEND, SLEEP_PROHIBIT_WAKE_LOCK_NAME);
		wake_lock(&sleep_prohibit_wake_lock);
#endif
		schedule_delayed_work(&work_expire_sleep_prohibit, SH_SLEEP_PROHIBIT_TIMEOUT);
		pr_info("start sleep prohibit\n");
	}
#endif
