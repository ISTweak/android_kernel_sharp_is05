...
#ifdef CONFIG_SHLOG_SYSTEM
void make_loginfo_hdr(void)
{
	static int once = 0;
	char *buf = NULL;
	unsigned long mask, len, end;
	...
	if(p_sh_smem_common_type == NULL) return;

	p_sh_smem_common_type->kp_magic = 0x77665590;
	buf = p_sh_smem_common_type->kp_buf;
	...
	while(len-- > 0) buf[len] = __log_buf[(end + len) & mask];

	return;
}
EXPORT_SYMBOL(make_loginfo_hdr); 
#endif
