...
#if 1 /* BT_001 Customize for Bluetooth */
#include <linux/gpio.h>

/* GPIO mapping for UART Tx/Rx */
#define UART_PORT_RX            136     /* UART1DM_RX_DATA */
#define UART_PORT_TX            137     /* UART1DM_TX_DATA */
#define HOST_WAKE_IN            142     /* HOST_WAKE_IN */

/* Port Configuration Table */
static unsigned uart_gpio_config[] = {
    /*        gpio,         func, dir,             pull,             drvstr */
    GPIO_CFG( UART_PORT_RX, 1,    GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
    GPIO_CFG( UART_PORT_TX, 1,    GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_6MA),
};
#endif /* BT_001 Customize for Bluetooth */
...
#if 1 /* BT_002 Customize for Bluetooth */
...
#endif /* BT_002 Customize for Bluetooth */
...
#else /* BT_002 Customize for Bluetooth */
	wake_lock(&msm_uport->dma_wake_lock);
#endif /* BT_002 Customize for Bluetooth */
...
#if 0 /* BT_001 Customize for Bluetooth */
...
static int msm_hs_check_clock_off_locked(struct uart_port *uport)
{
    ...
    for (pin = 0; pin < ARRAY_SIZE(uart_gpio_config); pin++) {
        result = gpio_tlmm_config( uart_gpio_config[pin], GPIO_CFG_DISABLE );
        if (result) {
...
static void msm_hs_request_clock_on_locked(struct uart_port *uport) {
	...
	case MSM_HS_CLK_OFF:
        ...
        /* Port Configuration */
        for (pin = 0; pin < ARRAY_SIZE(uart_gpio_config); pin++) {
            result = gpio_tlmm_config( uart_gpio_config[pin], GPIO_CFG_ENABLE );
            if (result) {
                printk(KERN_ERR  "gpio_tlmm_config(disable:%d)<-%#x : %d\n", pin, uart_gpio_config[pin], result);
            }
        }
...
static irqreturn_t msm_hs_wakeup_isr(int irq, void *dev)
{
	...
	if (msm_uport->clk_state == MSM_HS_CLK_OFF)  {
		 ...
		 * before enable_irq() */
#if 0 /* BT_003 Customize for Bluetooth */
		if (msm_uport->wakeup.ignore)
			msm_uport->wakeup.ignore = 0;
		else
			wakeup = 1;
#else /* BT_003 Customize for Bluetooth */
		if (msm_uport->wakeup.ignore) {
			msm_uport->wakeup.ignore = 0;
			wakeup = 1;
        }
#endif /* BT_003 Customize for Bluetooth */
	}
...
/* COORDINATOR Qualcomm404002 MERGE start */
#if 0
	ret = set_irq_wake(uport->irq, 1);
	if (unlikely(ret))
		return ret;
#endif
/* COORDINATOR Qualcomm404002 MERGE end */
