/* drivers/mtd/devices/msm_nand.c ( Nand Driver )
 *
...
#ifndef CONFIG_ANDROID_ENGINEERING
static void msm_nand_set_protect_part(struct platform_device *pdev,
					struct mtd_info *p_mtd)
{
	uint32_t i;
	uint32_t j;
	...
	for(j = 0; j < WRITE_PROTECT_AREA_NUM; j++) {
		for (i = 0; i < pdata->nr_parts; i++) {
			if(0 == strcmp(pdata->parts[i].name,
				protect_area_name[j])) {
				s_protect_info[j].start
					= pdata->parts[i].offset
					* p_mtd->erasesize;
				s_protect_info[j].end
					= pdata->parts[i].offset
					* p_mtd->erasesize
					+ pdata->parts[i].size
					* p_mtd->erasesize;
				s_protect_info[j].flg
					= 1; /* 0:invalid 1:valid */
				break;
			}
		}
...
static int msm_nand_check_protect_part(uint32_t start_adr, uint32_t end_adr)
{
	...
	for(j = 0; j < WRITE_PROTECT_AREA_NUM; j++) {
		if(s_protect_info[j].flg == 1) {
			/* start address check */
			if((s_protect_info[j].start <= start_adr)
			&& (start_adr < s_protect_info[j].end))
				return -EROFS;

			/* end address check */
			if((s_protect_info[j].start < end_adr)
			&& (end_adr <= s_protect_info[j].end))
				return -EROFS;
		}
	}
			...
			struct mtd_oob_ops *ops)
{
	...
	while (page_count-- > 0) {
		...
		for (n = start_sector; n < cwperpage; n++) {
			 ...
			 * (only valid if status says success)
			 */
#if 0	/* customize Read from only oob with MTD_OOB_PLACE mode */
			if (ops->datbuf) {
#else
...
#if 1	/* customize Read from only oob with MTD_OOB_PLACE mode */
					if (ops->datbuf) {
#endif
					cmd->cmd = 0;
		...
		if (rawerr) {
			...
			if (ops->oobbuf) {
/* COORDINATOR Qualcomm404012 MERGE start */
#if 1	/* customize for change ownership of DMA */
				dma_sync_single_for_cpu(chip->dev,
				  oob_dma_addr_curr - (ops->ooblen - oob_len),
				  ops->ooblen - oob_len, DMA_BIDIRECTIONAL);
#endif
				...
				dma_sync_single_for_device(chip->dev,
				  ...
				  ops->ooblen - oob_len, DMA_BIDIRECTIONAL);
#endif
/* COORDINATOR Qualcomm404012 MERGE end */
			}
		}
		if (pageerr) {
			for (n = start_sector; n < cwperpage; n++) {
	      ...
	      size_t *retlen, u_char *buf)
{
	...
	size_t nand_offset_len;
	size_t nand_part_len;
	size_t copy_len;
	size_t left_len;
#endif
	...
	if (nand_offset_len == 0 && nand_part_len == 0) {
		...
		*retlen = 0;
		...
		if (!work_nand_buf) {
			pr_err("msm_nand_read: "
				"failed to alloc work nand buffer\n");
			ret = -ENOMEM;
		...
		if (nand_offset_len != 0) {
			...
			if (!dual_nand_ctlr_present) {
				...
				ret = msm_nand_read_oob_dualnandc(mtd,
							from, &ops);
			}
		...
		if (left_len > mtd->writesize) {
			copy_len = (left_len / mtd->writesize)
					* mtd->writesize;
			ops.len = copy_len;
			ops.datbuf = buf;
