...
#include <../../../arch/arm/mach-msm/include/mach/rpc_server_handset.h>
...
#define KEYF_FIX_LAST_ROW		0x01
	...
	#define GND_KEY_ROW1		8		/* KEYOUT_9 */
	#define GND_KEY_ROW2		9		/* KEYOUT_10 */

	#define MATRIX_AF_ROW		4
	#define MATRIX_AF_COLUMN	6

	#define MATRIX_CAMERA_ROW		4
	#define MATRIX_CAMERA_COLUMN	7

#elif (defined(CONFIG_TENKEY_KEYPAD) || defined(CONFIG_TENKEY2_KEYPAD) || defined(CONFIG_TENKEY3_KEYPAD))

	#define MATRIX_AF_ROW		2
	#define MATRIX_AF_COLUMN	6

	#define MATRIX_CAMERA_ROW		2
	#define MATRIX_CAMERA_COLUMN	7

#else

	#define MATRIX_AF_ROW		0
	#define MATRIX_AF_COLUMN	6

	#define MATRIX_CAMERA_ROW		0
	#define MATRIX_CAMERA_COLUMN	7
	...
	#define SHDBG_KEY_HAL(__dbg)	__dbg
	#define SHDBG_KEY_HAL_LOGHIGH(__dbg)
	#define SHDBG_KEY_HAL_LOGLOW(__dbg)
...
#endif /* defined(CONFIG_TENKEY_KEYPAD) || defined(CONFIG_TENKEY2_KEYPAD) || defined(CONFIG_TENKEY3_KEYPAD) */
#if (defined(CONFIG_QWERTY_KEYPAD) || defined(CONFIG_QWERTY2_KEYPAD) || defined(CONFIG_QWERTY3_KEYPAD))
	SHDBG_KEY_HAL(printk("[SC][KEY][NEW_BF] :0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X\n"
									, new_state[0], new_state[1], new_state[2], new_state[3], new_state[4]
									, new_state[5], new_state[6], new_state[7], new_state[8], new_state[9]
									);)

	if (old_state)
	{
		if(board_state == 1) {
			old_state[GND_KEY_ROW1] = old_state[GND_KEY_ROW1] | 0xA8;
			old_state[GND_KEY_ROW2] = old_state[GND_KEY_ROW2] | 0x50;
		}
		bits_changed_old1 = old_state[GND_KEY_ROW1] | 0x57;
		bits_changed_old2 = old_state[GND_KEY_ROW2] | 0xA8;
		bits_changed_old1 = bits_changed_old1 & bits_changed_old2;
	
/*		if(bits_changed1 != 0xFF)	*/
/*		{							*/
			i=0x01;
			for (j = 0; j < 8; j++)
			{
				if ((bits_changed_old1 & i) == 0 ) 
				{
					for (k = 0; k < 8; k++)
					{
						old_state[k] = old_state[k] | i;
#if 0
						if ((i == 0x20) || (i == 0x80)) 
						{
							old_state[9] = old_state[9] | i;
						}
						else {
							old_state[8] = old_state[8] | i;
						}
#endif
					}
				}
...
/*		} */
		if( (old_state[GND_KEY_ROW1] | 0x7F) != 0xFF)
		{
			old_state[GND_KEY_ROW2] = old_state[GND_KEY_ROW2] | 0x80;
		}
		if( (old_state[GND_KEY_ROW1] | 0xDF) != 0xFF)
		{
			old_state[GND_KEY_ROW2] = old_state[GND_KEY_ROW2] | 0x20;
		}
		if( (old_state[GND_KEY_ROW2] | 0xBF) != 0xFF)
		{
			old_state[GND_KEY_ROW1] = old_state[GND_KEY_ROW1] | 0x40;
		}
		if( (old_state[GND_KEY_ROW2] | 0xEF) != 0xFF)
		{
			old_state[GND_KEY_ROW1] = old_state[GND_KEY_ROW1] | 0x10;
		}
		old_state[GND_KEY_ROW1] = old_state[GND_KEY_ROW1] | 0x03;

	}

	if(board_state == 1) {
		new_state[GND_KEY_ROW1] = new_state[GND_KEY_ROW1] | 0xA8;
		new_state[GND_KEY_ROW2] = new_state[GND_KEY_ROW2] | 0x50;
	}
	bits_changed1 = new_state[GND_KEY_ROW1] | 0x57;
	bits_changed2 = new_state[GND_KEY_ROW2] | 0xA8;
	bits_changed1 = bits_changed1 & bits_changed2;
	
	i=0x01;
	for (j = 0; j < 8; j++)
	{
		if ((bits_changed1 & i) == 0 ) 
		{
			for (k = 0; k < 8; k++)
			{
				new_state[k] = new_state[k] | i;
#if 0
				if ((i == 0x20) || (i == 0x80))
				{
					new_state[9] = new_state[9] | i;
				}
				else {
					new_state[8] = new_state[8] | i;
				}
#endif
			}
		}
	...
	if( (new_state[GND_KEY_ROW1] | 0x7F) != 0xFF)
	{
		new_state[GND_KEY_ROW2] = new_state[GND_KEY_ROW2] | 0x80;
	}
	if( (new_state[GND_KEY_ROW1] | 0xDF) != 0xFF)
	{
		new_state[GND_KEY_ROW2] = new_state[GND_KEY_ROW2] | 0x20;
	}
	if( (new_state[GND_KEY_ROW1] | 0xF7) != 0xFF)
	{
		new_state[GND_KEY_ROW2] = new_state[GND_KEY_ROW2] | 0x08;
	}
	if( (new_state[GND_KEY_ROW2] | 0xBF) != 0xFF)
	{
		new_state[GND_KEY_ROW1] = new_state[GND_KEY_ROW1] | 0x40;
	}
	if( (new_state[GND_KEY_ROW2] | 0xEF) != 0xFF)
	{
		new_state[GND_KEY_ROW1] = new_state[GND_KEY_ROW1] | 0x10;
	}
	if( (new_state[GND_KEY_ROW2] | 0xFB) != 0xFF)
	{
		new_state[GND_KEY_ROW1] = new_state[GND_KEY_ROW1] | 0x04;
	}
	new_state[GND_KEY_ROW1] = new_state[GND_KEY_ROW1] | 0x03;

	SHDBG_KEY_HAL(printk("[SC][KEY][NEW_AF] :0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X\n"
									, new_state[0], new_state[1], new_state[2], new_state[3], new_state[4]
									, new_state[5], new_state[6], new_state[7], new_state[8], new_state[9]
									);)
...
{
#ifdef CONFIG_KEYBOARD_SCKEY
#if ( defined(CONFIG_QWERTY_KEYPAD) || defined(CONFIG_QWERTY2_KEYPAD) || defined(CONFIG_QWERTY3_KEYPAD) || defined(CONFIG_TENKEY_KEYPAD)|| defined(CONFIG_TENKEY2_KEYPAD)|| defined(CONFIG_TENKEY3_KEYPAD) )
	int row = 0;
	int CodeSendFlag = 0;
	int col, code;
	int code_focus;
	...
	u16 state_camera;
//#if 0
	shextdet_form_position_result_t state_position;
//#endif
	/* use matrix */
	for (row = 0; row < kp->pdata->num_rows; row++) {
		...
		for (col = 0; col < kp->pdata->num_cols; col++) {
			...
			code = (row << 3) + col;
			if(kp->keycodes[code] == KEY_RESERVED) {
				continue;
			}
			
#if (defined(CONFIG_TENKEY_KEYPAD) || defined(CONFIG_TENKEY2_KEYPAD) || defined(CONFIG_TENKEY3_KEYPAD))
#if 0
			if(   (kp->keycodes[code] != KEY_SAVE)
			   && (kp->keycodes[code] != KEY_FOCUS)
			   && (kp->keycodes[code] != KEY_CAMERA)
			   && (kp->keycodes[code] != KEY_VOLUMEDOWN)
			   && (kp->keycodes[code] != KEY_VOLUMEUP )  ) {

				state_position = shextdet_get_form_state();
				if(  (state_position==SHEXTDET_FORM_POSITION_CLOSE)
				  || (state_position==SHEXTDET_FORM_POSITION_SWIVEL)) {
					SHDBG_KEY_HAL(printk(KERN_INFO "[shkey]slide close\n");)
					/* close */
					if( !(new_state[row] & (1 << col))) {
						SHDBG_KEY_HAL(printk(KERN_INFO "[shkey]keycode not send\n");)
						continue;
					}
				}
			}
#endif
			/* FOCUS and CAMERA key check */
			state_focus = !(new_state[MATRIX_AF_ROW] & (1 << MATRIX_AF_COLUMN));
			state_camera = !(new_state[MATRIX_CAMERA_ROW] & (1 << MATRIX_CAMERA_COLUMN));
			code_focus = MATRIX_SCAN_CODE(MATRIX_AF_ROW, MATRIX_AF_COLUMN, PM8058_ROW_SHIFT);
			if((kp->keycodes[code] == KEY_FOCUS) && (row == MATRIX_AF_ROW)) {
				/* Shutter KEY on */
				if( state_camera ) {
					if( !(bits_changed & (1 << MATRIX_CAMERA_COLUMN))) {
						continue;
					}
				}else if( (bits_changed & (1 << MATRIX_CAMERA_COLUMN))) {
						continue;
				}
			}else if((kp->keycodes[code] == KEY_CAMERA) && (row == MATRIX_CAMERA_ROW)) {
				if( (state_focus == 0) && (state_camera == 0) ) {
					...
					SHDBG_KEY_HAL(printk(KERN_INFO "[shkey]keycode:%d,code:%d,sts:0x%x,row:0x%x,col:0x%x\n"
										, kp->keycodes[code]
										, code
										, !(new_state[row] & (1 << col))
										, row
					...
					/* FOCUS */
					input_event(kp->input, EV_MSC, MSC_SCAN, code_focus);
					input_report_key(kp->input,
									kp->keycodes[code_focus],
									state_focus);
				...
				}else if( (state_focus == 0) && (state_camera != 0) ) {
					CodeSendFlag = 1;
					/* FOCUS */
					input_event(kp->input, EV_MSC, MSC_SCAN, code_focus);
					input_report_key(kp->input,
...
#if (defined(CONFIG_QWERTY_KEYPAD) || defined(CONFIG_QWERTY2_KEYPAD) || defined(CONFIG_QWERTY3_KEYPAD))
			if(   (kp->keycodes[code] != KEY_MENU)
			   && (kp->keycodes[code] != KEY_BACK)
			   && (kp->keycodes[code] != KEY_HOME)
			   && (kp->keycodes[code] != KEY_SEARCH)
			   && (kp->keycodes[code] != KEY_FOCUS)
			   && (kp->keycodes[code] != KEY_CAMERA)
			   && (kp->keycodes[code] != KEY_VOLUMEDOWN)
			   && (kp->keycodes[code] != KEY_VOLUMEUP )  ) {
			...
			SHDBG_KEY_HAL(printk("[SC][KEY]keycode:%d,code:%d,sts:0x%x,row:0x%x,col:0x%x\n"
										, kp->keycodes[code]
										, code
										, !(new_state[row] & (1 << col))
										, row
	...
	if (CodeSendFlag >0) {
		input_sync(kp->input);
		SHDBG_KEY_HAL(printk("[SC][KEY]input_sync output\n");)
	}
	return 0;
#else	/* defined(CONFIG_QWERTY_KEYPAD) || defined(CONFIG_QWERTY2_KEYPAD) || defined(CONFIG_QWERTY3_KEYPAD) || defined(CONFIG_TENKEY_KEYPAD) || defined(CONFIG_TENKEY2_KEYPAD) || defined(CONFIG_TENKEY3_KEYPAD) */
	int row = 0;
	int CodeSendFlag = 0;
	int col, code;
	int code_focus;
	...
	return 0;
#endif	/* defined(CONFIG_QWERTY_KEYPAD) || defined(CONFIG_QWERTY2_KEYPAD) || defined(CONFIG_QWERTY3_KEYPAD) || defined(CONFIG_TENKEY_KEYPAD) || defined(CONFIG_TENKEY2_KEYPAD) || defined(CONFIG_TENKEY3_KEYPAD) */
...
#endif
//	printk("[SC][KEY]pmic8058_kp_stuck_irq()\n");
