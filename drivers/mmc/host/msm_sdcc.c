...
#if defined(CONFIG_MMC_CUST_SH) && defined(CONFIG_MMC_MSM_SDC2_SUPPORT)
#define CONFIG_IDLECTRL_EMMC_CUST_SH
#endif
...
struct mmc_host *msmsdcc_idle_mmc0;
unsigned int  msmsdcc_state_suspend_mmc0 = 0;
unsigned int  msmsdcc_state_idle_mmc0 = 0;
#endif /* CONFIG_SDIO_CUST_SH */
#if defined(CONFIG_SDIO_CUST_SH) || defined(CONFIG_IDLECTRL_EMMC_CUST_SH)
static void msmsdcc_pm_idle_suspend_lower(struct mmc_host *mmc);
static void msmsdcc_pm_idle_resume_lower(struct mmc_host *mmc);
#endif /* CONFIG_SDIO_CUST_SH || CONFIG_IDLECTRL_EMMC_CUST_SH */
#ifdef CONFIG_IDLECTRL_EMMC_CUST_SH
static struct semaphore	msmsdcc_suspendresume_sem_mmc1;
struct mmc_host *msmsdcc_idle_mmc1;
unsigned int  msmsdcc_state_suspend_mmc1 = 0;
unsigned int  msmsdcc_state_idle_mmc1 = 0;
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if( strncmp(mmc_hostname(mmc),"mmc0",4) == 0){
		...
		msmsdcc_idle_mmc0 = mmc;
		
		if(plat->sdiowakeup_irq){
		}else{
			plat->sdiowakeup_irq = MSM_GPIO_TO_INT(42);
		}
		
	}
	/* SH cust E*/
#ifdef CONFIG_IDLECTRL_EMMC_CUST_SH
	if( strncmp(mmc_hostname(mmc),"mmc1",4) == 0){
		init_MUTEX(&msmsdcc_suspendresume_sem_mmc1);
		msmsdcc_idle_mmc1 = mmc;
	}
#endif /* CONFIG_IDLECTRL_EMMC_CUST_SH */
...
static int msmsdcc_pm_suspend(struct device *dev)
{
	...
	if( strncmp(mmc_hostname(mmc),"mmc0",4) == 0){
		...
		msmsdcc_state_suspend_mmc0 = 1;
		
		if(msmsdcc_state_idle_mmc0 != 0){
			msmsdcc_pm_idle_resume_lower(msmsdcc_idle_mmc0);
		}
...
#ifdef CONFIG_IDLECTRL_EMMC_CUST_SH
	if( strncmp(mmc_hostname(mmc),"mmc1",4) == 0){
		down(&msmsdcc_suspendresume_sem_mmc1);
		msmsdcc_state_suspend_mmc1 = 1;
		
		if(msmsdcc_state_idle_mmc1 != 0){
			msmsdcc_pm_idle_resume_lower(msmsdcc_idle_mmc1);
		}
		
		up(&msmsdcc_suspendresume_sem_mmc1);
	}
#endif /* CONFIG_IDLECTRL_EMMC_CUST_SH */
...
static int msmsdcc_pm_resume(struct device *dev)
{
	...
	if( strncmp(mmc_hostname(mmc),"mmc0",4) == 0){
		...
		msmsdcc_state_suspend_mmc0 = 0;
	}
#endif
#ifdef CONFIG_IDLECTRL_EMMC_CUST_SH
	if( strncmp(mmc_hostname(mmc),"mmc1",4) == 0){
		down(&msmsdcc_suspendresume_sem_mmc1);
		msmsdcc_state_suspend_mmc1 = 0;
		up(&msmsdcc_suspendresume_sem_mmc1);
	}
#endif /* CONFIG_IDLECTRL_EMMC_CUST_SH */
	return rc;
...
#if defined(CONFIG_SDIO_CUST_SH) || defined(CONFIG_IDLECTRL_EMMC_CUST_SH)
void msmsdcc_pm_idle_suspend(void){
#ifdef CONFIG_SDIO_CUST_SH

	if(msmsdcc_state_suspend_mmc0 != 0){
		return;
	}
	msmsdcc_state_idle_mmc0 = 1;

	msmsdcc_pm_idle_suspend_lower(msmsdcc_idle_mmc0);
#endif /* CONFIG_SDIO_CUST_SH */
#ifdef CONFIG_IDLECTRL_EMMC_CUST_SH
	down(&msmsdcc_suspendresume_sem_mmc1);
	if(msmsdcc_state_suspend_mmc1 != 0){
		up(&msmsdcc_suspendresume_sem_mmc1);
		return;
	}
	msmsdcc_state_idle_mmc1 = 1;
	up(&msmsdcc_suspendresume_sem_mmc1);

	msmsdcc_pm_idle_suspend_lower(msmsdcc_idle_mmc1);
#endif /* CONFIG_IDLECTRL_EMMC_CUST_SH */
}
...
void msmsdcc_pm_idle_resume(void){
#ifdef CONFIG_SDIO_CUST_SH
