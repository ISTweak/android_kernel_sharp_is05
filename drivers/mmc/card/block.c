...
static int mmc_blk_issue_rq(struct mmc_queue *mq, struct request *req)
{
	...
	int rel_wr_on = 0;
	int en_rel_wr_val;
	int rel_wr_sec_c_val;
	int hpi_support_val;
	
	en_rel_wr_val = ext_csd_en_rel_wr_val;
	rel_wr_sec_c_val = ext_csd_rel_wr_sec_c_val;
	hpi_support_val = ext_csd_hpi_support_val;
	/* SH cust E */
#endif
...
#if 1
		/* SH cust S */
		if ((rq_data_dir(req) == WRITE) && (brq.data.blocks <= 32)) {
			if( (brq.cmd.arg / brq.data.blksz / 16) < ( (brq.cmd.arg / brq.data.blksz ) + brq.data.blocks - 1) / 16) {
				brq.data.blocks = 16 - ((brq.cmd.arg / brq.data.blksz) % 16);
			}
		}
		/* SH cust E */
#endif
