 ...
 *
 * This code is based on mdp4_dtv.c.
...
static int pmdh_dtv_clk_status = 0;
void pmdh_dtv_clk_disable(void)
{
	if (pmdh_dtv_clk_status == 0) {
		return;
	}

	if (tv_enc_clk) {
		pmdh_dtv_clk_status = 0;
...
void pmdh_dtv_clk_enable(void)
{
	if (pmdh_dtv_clk_status == 1) {
		return;
	}

	if (tv_enc_clk) {
		pmdh_dtv_clk_status = 1;
...
#if defined(CONFIG_SHLCDC_BOARD) && defined(ENABLE_PMDH_CLK_GATING)
	pmdh_dtv_clk_disable();
#else	/* CONFIG_SHLCDC_BOARD && ENABLE_PMDH_CLK_GATING */
	clk_disable(tv_enc_clk);
	clk_disable(tv_dac_clk);
	clk_disable(hdmi_clk);
	if (mdp_tv_clk)
