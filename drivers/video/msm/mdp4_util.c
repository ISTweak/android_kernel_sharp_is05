 ...
 *
 * This code is based on mdp4_util.c.
...
#ifdef CONFIG_SHLCDC_BOARD
static uint32 csc_matrix_tab_BT709[9] = {
	0x0254, 0x0000, 0x03aa,
	0x0254, 0xff90, 0xfee9,
	0x0254, 0x0452, 0x0000
};

static uint32 csc_pre_bv_tab_BT709[3] = {0xfff0, 0xff80, 0xff80 };
static uint32 csc_pre_lv_tab_BT709[6] = {0x10, 0xeb, 0x10, 0xf0, 0x10, 0xf0 };
#endif /* CONFIG_SHLCDC_BOARD */
...
void mdp4_vg_csc_mv_setup_BT709(int vp_num)
{
	...
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	for (i = 0; i < 9; i++) {
		outpdw(off, csc_matrix_tab_BT709[i]);
		off++;
	}
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
}
...
void mdp4_vg_csc_pre_bv_setup_BT709(int vp_num)
{
	...
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	for (i = 0; i < 3; i++) {
		outpdw(off, csc_pre_bv_tab_BT709[i]);
		off++;
	}
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
}
...
void mdp4_vg_csc_pre_lv_setup_BT709(int vp_num)
{
	...
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	for (i = 0; i < 6; i++) {
		outpdw(off, csc_pre_lv_tab_BT709[i]);
		off++;
	}
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
}
...
{
	if( matrix == MDP_CCS_YUV_BT709 ){
		mdp4_vg_csc_mv_setup_BT709(0);
		mdp4_vg_csc_mv_setup_BT709(1);
		mdp4_vg_csc_pre_bv_setup_BT709(0);
		mdp4_vg_csc_pre_bv_setup_BT709(1);
		mdp4_vg_csc_pre_lv_setup_BT709(0);
		mdp4_vg_csc_pre_lv_setup_BT709(1);
	}
	else{
		mdp4_vg_csc_mv_setup(0);
		mdp4_vg_csc_mv_setup(1);
