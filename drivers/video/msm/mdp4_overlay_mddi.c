 ...
 *
 * This code is based on mdp4_overlay_mddi.c.
...
#ifdef CONFIG_SHLCDC_BOARD
int master4_invalid_flg = 0;
int master4_invalid_status = 0;
#endif /*CONFIG_SHLCDC_BOARD*/
...
void mdp4_mddi_overlay_wait_finish(struct msm_fb_data_type *mfd)
{
	...
	spin_lock_irqsave(&mdp_spin_lock, flag);
	if (mfd->dma->busy == TRUE) {
		INIT_COMPLETION(pending_pipe->comp);
	}
	spin_unlock_irqrestore(&mdp_spin_lock, flag);
	
	if (mfd->dma->busy == TRUE) {
		mutex_unlock(&mfd->dma->ov_mutex);
		wait_for_completion_killable(&pending_pipe->comp);
		mutex_lock(&mfd->dma->ov_mutex);
	}
}
#endif /* CONFIG_SHLCDC_BOARD */
