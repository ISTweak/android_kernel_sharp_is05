...
static int __init mddi_sharp_init(void)
{
	...
	if (mddi_host_core_version > 8) {
		...
		/* dot_clock = 24.53MHz
		 * horizontal count = 504 (480 + 8 + 8 + 8)
		 * vertical count = 864 (854 + 2 + 2 + 6)
		 * refresh rate = 24,533,000/(504*864) = 56.339Hz
		 */
		mddi_sharp_rows_per_second = 48677;	/* 24,533,000/504 */
		mddi_sharp_rows_per_refresh = 864;
		mddi_sharp_usecs_per_refresh = 17750;	/* (504*864)/24,533,000 */
#else
		/* Timing variables for tracking vsync */
		/* dot_clock = 6.00MHz
	...
	if (!ret) {
		...
		pinfo->xres = CONFIG_SHLCDC_PANEL_SIZE_WIDTH;
		pinfo->yres = CONFIG_SHLCDC_PANEL_SIZE_HEIGHT;
