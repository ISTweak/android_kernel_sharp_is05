 ...
 *
 * This code is based on mdp4_overlay_dtv.c.
...
int mdp4_dtv_on(struct platform_device *pdev)
{
...
#ifdef CONFIG_SHLCDC_BOARD
	MDP_OUTP(MDP_BASE + DTV_BASE + 0x58, 0x00000320);
#endif /*CONFIG_SHLCDC_BOARD*/
	...
	if (ret == 0) {
		...
		#endif
		#if defined(CONFIG_SHLCDC_BOARD) && !defined(ENABLE_PMDH_CLK_GATING)
		mdp_pipe_ctrl(MDP_OVERLAY1_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
		#endif /* CONFIG_SHLCDC_BOARD && !ENABLE_PMDH_CLK_GATING */
...
void mdp4_dtv_overlay(struct msm_fb_data_type *mfd)
{
	...
	if(mfd->dma->waiting == TRUE){
		...
		down(&mfd->sem);
		up(&mfd->sem);
		#ifdef ENABLE_PMDH_CLK_GATING
		mdp_pipe_ctrl(MDP_OVERLAY1_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
		#endif /* ENABLE_PMDH_CLK_GATING */
		return;
	}
	...
	if (!wait_for_completion_timeout(&dtv_pipe->comp, msecs_to_jiffies(100))) {
...
void mdp4_dtv_overlay_regflush(struct mdp4_overlay_pipe *pipe, struct msm_fb_data_type *mfd, int all)
{
	...
	mdp4_overlay_reg_flush(pipe, all);
