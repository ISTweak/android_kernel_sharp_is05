 ...
 *
 * This code is based on mdp4_overlay.c.
...
extern int master4_invalid_flg;
extern int master4_invalid_status;
...
#ifdef CONFIG_SHLCDC_BOARD
	if(id == ctrl->stage[MDP4_MIXER0][MDP4_MIXER_STAGE_BASE]->pipe_ndx){
		return 0;
	}
#endif /*CONFIG_SHLCDC_BOARD*/
...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
...
#ifdef CONFIG_SHLCDC_BOARD
	if(req->id == ctrl->stage[MDP4_MIXER0][MDP4_MIXER_STAGE_BASE]->pipe_ndx){
		if ((mfd->panel_info.xres != req->dst_rect.w)
			||(mfd->panel_info.yres != req->dst_rect.h)
			||(req->dst_rect.x != 0)
			||(req->dst_rect.y != 0)){
			mutex_unlock(&mfd->dma->ov_mutex);
			return -ERANGE;
...
#ifdef CONFIG_SHLCDC_BOARD
	if ( ((req->src_rect.h > req->dst_rect.h) || (req->src_rect.w > req->dst_rect.w)) &&
		 (mdp4_overlay_format2type(req->src.format) == OVERLAY_TYPE_VIDEO) ) {
			perf_level = OVERLAY_PERF_LEVEL1;
	}
#endif /*CONFIG_SHLCDC_BOARD*/

#ifdef CONFIG_SHLCDC_BOARD
	if(mdp4_overlay_format2type(req->src.format) == OVERLAY_TYPE_VIDEO){
		if(req->dst_rect.w < (req->src_rect.w * 9 / 20)){
			master4_invalid_flg = 1;
		}else{
			master4_invalid_flg = 0;
		}
	}
#endif /*CONFIG_SHLCDC_BOARD*/
...
#ifdef CONFIG_SHLCDC_BOARD
	if((pipe->pipe_ndx == 3) || (pipe->pipe_ndx == 4) || (pipe->pipe_ndx == 5) || (pipe->pipe_ndx == 6)){
		if(req->id == MSMFB_NEW_REQUEST){
			extern void mdp4_overlay_mdp_reset(unsigned char reset);

			if ( pipe->mixer_num == MDP4_MIXER0 ){
				mdp4_mddi_overlay_wait_finish(mfd);
			}
			else{
				mdp4_dtv_overlay_wait_finish(mfd);
			}
			
			/* reset mixer 0 before it's use this will make sure that mixer 0 is reset
			   before switch from mixer 1 to mixer 0 and vice versa*/
			if (pipe->mixer_num == MDP4_MIXER1){
				mdp4_overlay_mdp_reset(2); 
			}else if (pipe->mixer_num == MDP4_MIXER0){
				mdp4_overlay_mdp_reset(1);
			}
		}
	}
#endif /*CONFIG_SHLCDC_BOARD*/
...
#ifdef CONFIG_SHLCDC_BOARD
	{
		...
		if(mixer == MDP4_MIXER1 && mdp4_overlay_format2type(req->src.format) == OVERLAY_TYPE_VIDEO){
			tv_src_clk = clk_get(NULL, "tv_src_clk");
			ret = clk_set_rate(tv_src_clk, 49500000);
			if (ret) {
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
...
#ifdef CONFIG_SHLCDC_BOARD
	{
		int ret;
		static struct clk *tv_src_clk;
		if(pipe->mixer_num == MDP4_MIXER1 && pipe->pipe_type == OVERLAY_TYPE_VIDEO){
			tv_src_clk = clk_get(NULL, "tv_src_clk");
			ret = clk_set_rate(tv_src_clk, 37125000);
			if (ret) {
				pr_info("%s: clk_set_rate(%d) failed\n", __func__,
					mfd->fbi->var.pixclock);
			}
...
#ifdef CONFIG_SHLCDC_BOARD
	if((mdp_hw_revision == MDP4_REVISION_V2_1) && master4_invalid_flg && (mdp4_overlay_ndx2pipe(5)==NULL) && (mdp4_overlay_ndx2pipe(6)==NULL)){
		uint32	data;
		...
		struct file **pp_src_file)
{
	...
	*pp_src_file = p_src_file;
	...
	if ( pipe->mixer_num == MDP4_MIXER1 ){
		ctrl->mixer1_played++;
		mdp4_dtv_overlay_regflush(pipe, mfd, 1);
	}
	else { 	/* MDDI mode */
		ctrl->mixer0_played++;
		if (mfd->panel_power_on)
...
#ifdef CONFIG_SHLCDC_BOARD
int mdp4_overlay_play_w(struct fb_info *info, struct msmfb_overlay_data_w *req,
		struct file **pp_src_file1, struct file **pp_src_file2)
{
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	struct msmfb_data *img;
	struct mdp4_overlay_pipe *pipe1, *pipe2;
	struct mdp4_pipe_desc *pd;
	ulong start, addr;
	ulong len = 0;
	struct file *p_src_file = 0;
	...
	pipe1 = mdp4_overlay_ndx2pipe(req->id1);
	if (pipe1 == NULL)
		return -ENODEV;

	pipe2 = mdp4_overlay_ndx2pipe(req->id2);
	if (pipe2 == NULL)
		return -ENODEV;
	...
	pd = &ctrl->ov_pipe[pipe1->pipe_num];
	if (pd->player && pipe1 != pd->player) {
		if (pipe1->pipe_type == OVERLAY_TYPE_RGB) {
			mutex_unlock(&mfd->dma->ov_mutex);
			return 0; /* ignore it, kicked out already */
		}
	}
	pd->player = pipe1;	/* keep */

	pd = &ctrl->ov_pipe[pipe2->pipe_num];
	if (pd->player && pipe2 != pd->player) {
		if (pipe2->pipe_type == OVERLAY_TYPE_RGB) {
			mutex_unlock(&mfd->dma->ov_mutex);
			return 0; /* ignore it, kicked out already */
		}
	}
	pd->player = pipe2;	/* keep */
	...
	*pp_src_file1 = p_src_file;

	addr = start + img->offset;
	pipe1->srcp0_addr = addr;
	pipe1->srcp0_ystride = pipe1->src_width * pipe1->bpp;

	if (pipe1->fetch_plane == OVERLAY_PLANE_PSEUDO_PLANAR) {
		if (pipe1->frame_format == MDP4_FRAME_FORMAT_VIDEO_SUPERTILE) {
			struct tile_desc tile;

			tile_samsung(&tile);
			pipe1->srcp1_addr = addr + tile_mem_size(pipe1, &tile);
		} else
			pipe1->srcp1_addr = addr +
					pipe1->src_width * pipe1->src_height;

		pipe1->srcp0_ystride = pipe1->src_width;
		pipe1->srcp1_ystride = pipe1->src_width;
	} else if (pipe1->fetch_plane == OVERLAY_PLANE_PLANAR) {
		addr += pipe1->src_width * pipe1->src_height;
		pipe1->srcp1_addr = addr;
		addr += ((pipe1->src_width / 2) * (pipe1->src_height / 2));
		pipe1->srcp2_addr = addr;
		pipe1->srcp0_ystride = pipe1->src_width;
		pipe1->srcp1_ystride = pipe1->src_width / 2;
		pipe1->srcp2_ystride = pipe1->src_width / 2;
	}

	if( req->data1.memory_id != req->data2.memory_id ){
		img = &req->data2;
		get_img(img, info, &start, &len, &p_src_file);
		if (len == 0) {
			mutex_unlock(&mfd->dma->ov_mutex);
	...
	addr = start + img->offset;
	pipe2->srcp0_addr = addr;
	pipe2->srcp0_ystride = pipe2->src_width * pipe2->bpp;

	if (pipe2->fetch_plane == OVERLAY_PLANE_PSEUDO_PLANAR) {
		if (pipe2->frame_format == MDP4_FRAME_FORMAT_VIDEO_SUPERTILE) {
			struct tile_desc tile;

			tile_samsung(&tile);
			pipe2->srcp1_addr = addr + tile_mem_size(pipe2, &tile);
		} else
			pipe2->srcp1_addr = addr +
					pipe2->src_width * pipe2->src_height;

		pipe2->srcp0_ystride = pipe2->src_width;
		pipe2->srcp1_ystride = pipe2->src_width;
	} else if (pipe2->fetch_plane == OVERLAY_PLANE_PLANAR) {
		addr += pipe2->src_width * pipe2->src_height;
		pipe2->srcp1_addr = addr;
		addr += ((pipe2->src_width / 2) * (pipe2->src_height / 2));
		pipe2->srcp2_addr = addr;
		pipe2->srcp0_ystride = pipe2->src_width;
		pipe2->srcp1_ystride = pipe2->src_width / 2;
		pipe2->srcp2_ystride = pipe2->src_width / 2;
	}

	if (pipe1->pipe_num >= OVERLAY_PIPE_VG1)
		mdp4_overlay_vg_setup(pipe1);	/* video/graphic pipe */
	else
		mdp4_overlay_rgb_setup(pipe1);	/* rgb pipe */
		
	mdp4_mixer_blend_setup(pipe1);
	mdp4_mixer_stage_up(pipe1);

	if (pipe2->pipe_num >= OVERLAY_PIPE_VG1)
		mdp4_overlay_vg_setup(pipe2);	/* video/graphic pipe */
	else
		mdp4_overlay_rgb_setup(pipe2);	/* rgb pipe */

	mdp4_mixer_blend_setup(pipe2);
	mdp4_mixer_stage_up(pipe2);

	if ( pipe1->mixer_num == MDP4_MIXER1 ){
		ctrl->mixer1_played++;
		mdp4_dtv_overlay_regflush(pipe1, mfd, 2);
	}
	else { 	/* MDDI mode */
		ctrl->mixer0_played++;
		if (mfd->panel_power_on)
			mdp4_mddi_overlay_kickoff(mfd, pipe1);

	}

	mdp4_stat.overlay_play[pipe1->mixer_num]++;
	mdp4_stat.overlay_play[pipe2->mixer_num]++;
...
#ifdef CONFIG_SHLCDC_BOARD
void mdp4_overlay_mdp_reset(unsigned char reset)
{
	struct mdp4_overlay_pipe *pipe;
	...
	switch(reset){
		...
		case 2:	/* mixer1 */
			...
			pipe = mdp4_overlay_ndx2pipe(2);
			mdp4_overlayproc_cfg(pipe);
			mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
			outpdw(MDP_BASE + 0x18000, 0x02);	/* MDP_OVERLAY_REG_FLUSH */
			mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
			break;
		case 3:	/* DMA_P */
			break;
