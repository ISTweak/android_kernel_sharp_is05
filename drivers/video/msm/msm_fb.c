...
int shdisp_upper_unit_is_connected = FALSE;
...
static void msm_fb_imageblit(struct fb_info *info, const struct fb_image *image)
{
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
#ifdef CONFIG_SHLCDC_BOARD
	extern unsigned char lcd_update_flg;
#endif /*CONFIG_SHLCDC_BOARD*/
...
#ifdef CONFIG_SHLCDC_BOARD
	var->height = CONFIG_SHLCDC_ACTIVE_AREA_HEIGHT;
	var->width = CONFIG_SHLCDC_ACTIVE_AREA_WIDTH;
...
#ifdef CONFIG_SHLCDC_BOARD
    if (msm_fb_suspend_flag_shdisp == TRUE) {
		return 0;
	}
#endif
...
#ifdef CONFIG_SHLCDC_BOARD
static int msmfb_overlay_play_w(struct fb_info *info, unsigned long *argp)
{
	int	ret;
	struct msmfb_overlay_data_w req;
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	struct file *p_src_file1 = 0;
	struct file *p_src_file2 = 0;
	...
	ret = mdp4_overlay_play_w(info, &req, &p_src_file1, &p_src_file2);

#ifdef CONFIG_ANDROID_PMEM
	if (p_src_file1)
		put_pmem_file(p_src_file1);
	if (p_src_file2)
		put_pmem_file(p_src_file2);
#endif

	return ret;
}

static int msmfb_pan_enable(struct fb_info *info, unsigned long *argp)
{
	int	ret, enable;
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
	...
	if (ret) {
		printk(KERN_ERR "%s:msmfb_pan_enable ioctl failed \n",
			__func__);
		return ret;
	}

	mfd->pan_enable = enable;
			...
			unsigned long arg)
{
	...
	case MSMFB_OVERLAY_PLAY:
		...
		break;
#ifdef CONFIG_SHLCDC_BOARD
	case MSMFB_OVERLAY_PLAY_W:
		down(&msm_fb_ioctl_ppp_sem);
		ret = msmfb_overlay_play_w(info, argp);
		up(&msm_fb_ioctl_ppp_sem);
		break;
	case MSMFB_PAN_ENABLE:
		down(&msm_fb_ioctl_ppp_sem);
		ret = msmfb_pan_enable(info, argp);
		up(&msm_fb_ioctl_ppp_sem);
		break;
#endif /*CONFIG_SHLCDC_BOARD*/
	...
	case MSMFB_SET_CCS_MATRIX:
...
#ifdef CONFIG_SHLCDC_BOARD
		{
			...
			down(&msm_fb_ioctl_ppp_sem);
			mdp4_vg_change_csc_matrix( ccs_matrix.direction );
			up(&msm_fb_ioctl_ppp_sem);
		}
#else
		ret = -EINVAL;
...
int __init msm_fb_init(void)
{
    ...
    shdisp_upper_unit_is_connected = shdisp_boot_ctx.upper_unit_is_connected;
#endif
