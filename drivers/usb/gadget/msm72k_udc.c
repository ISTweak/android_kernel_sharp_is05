...
#if defined(CONFIG_USB_ANDROID_SH_MTP) && defined(D_USB_GADGET_SETUP_PENDING)
	#define D_USB_GADGET_SETUP_DELAYSEND
#endif 

#ifdef CONFIG_USB_MSM_OTG_72K_SH_CUST
	#define D_USB_GADGET_NO_CHG_DET
#endif /* D_USB_GADGET_NO_CHG_DET */
...
#ifndef D_USB_GADGET_NO_CHG_DET
...
#define ep0in  ept[16]
	...
	atomic_t prevent_pullup;
	atomic_t self_powered_req;
#endif /* CONFIG_USB_ANDROID_SH_CUST */

#ifdef D_USB_GADGET_SETUP_DELAYSEND
	struct delayed_work	setup0_to_work;
#endif /* D_USB_GADGET_SETUP_DELAYSEND */
...
#define D_USB_GADGET_SETUP_PENDING_TIME		(5*HZ)
#define D_USB_GADGET_SETUP_SEND_MAX_ID		(32)

static struct usb_info *delay_ui = NULL;
static u8	delay_responce_send_cnt = 0;
static u8	delay_responce_wait_cnt = 0;

#define M_EP0_SETUP_DELAY_WAIT(x)		{delay_ui = x;delay_responce_wait_cnt++;}
#define M_EP0_SETUP_DELAY_DONE()		 \
						{	delay_ui = NULL;												 \
							if(++delay_responce_send_cnt > D_USB_GADGET_SETUP_SEND_MAX_ID)	 \
								delay_responce_send_cnt=0;									 \
							delay_responce_wait_cnt = delay_responce_send_cnt;				 \
						}
#define M_EP0_SETUP_DELAY_IS_OK()		(delay_responce_wait_cnt == (delay_responce_send_cnt + 1))

static void ep0_setup_wait_delay_response(struct usb_info *ui)
{
	//store usb_info
	M_EP0_SETUP_DELAY_WAIT(ui);

	//start delay_work_que
	schedule_delayed_work(&delay_ui->setup0_to_work,D_USB_GADGET_SETUP_PENDING_TIME);
...
void ep0_setup_send_delay_response(u8 *buf ,unsigned length)
{
	...
	/* stop delay_work_queue */
	if (!cancel_delayed_work_sync(&delay_ui->setup0_to_work)) {
		pr_err("ep0_setup_send_delay_response:cant stop work\n");
		return;
	}

	if (!M_EP0_SETUP_DELAY_IS_OK()) {
		pr_err("ep0_setup_send_delay_response:mismatch cnt\n");
		return;
	}


	/* send setup0 */
	req = delay_ui->setup_req;
	
	/* check the length of req->buf */ 
	if (length > SETUP_BUF_SIZE)
		length = SETUP_BUF_SIZE;
	
	memcpy( req->buf, buf, length);
	ep0_setup_send(delay_ui,length);
	M_EP0_SETUP_DELAY_DONE();

	return;
}

static void ep0_setup_delay_response_timemout(struct work_struct *w)
{
	/* this function for timeout */
	ep0_setup_stall(delay_ui);
	M_EP0_SETUP_DELAY_DONE();

	return;
}

static int ep0_setup_cancel_delay_response(void)
{
	if (!delay_ui)
		return 0;
...
static void handle_setup(struct usb_info *ui)
{
...
#endif
#ifdef D_USB_GADGET_SETUP_DELAYSEND
	if (ep0_setup_cancel_delay_response()) {
		/* stall against conflict between setup0 and response of before setup0 */
		goto stall;
	}
#endif /* D_USB_GADGET_SETUP_DELAYSEND */
	...
	if (ui->driver) {
		...
		if (ret >= 0)
			return;
#ifdef D_USB_GADGET_SETUP_DELAYSEND
		if (ret == D_USB_GADGET_SETUP_PENDING) {
			/* delay setup_stall for fail-safe */
			ep0_setup_wait_delay_response(ui);
			return;
		}
#endif /* D_USB_GADGET_SETUP_DELAYSEND */
	}
...
#ifdef D_USB_GADGET_SETUP_DELAYSEND
	INIT_DELAYED_WORK(&ui->setup0_to_work, ep0_setup_delay_response_timemout);
#endif /* D_USB_GADGET_SETUP_DELAYSEND */
}
...
static int msm72k_pullup_internal(struct usb_gadget *_gadget, int is_active)
{
	...
	if (is_active) {
		...
		if (is_usb_online(ui) && ui->driver && 
				!atomic_read(&ui->prevent_pullup)) {
			if (ui->fs_connect_enable) {
			...
			atomic_set(&ui->self_powered, atomic_read(&ui->self_powered_req));
			writel(readl(USB_USBCMD) | USBCMD_RS, USB_USBCMD);
		}
#else /* CONFIG_USB_ANDROID_SH_CUST */
		if (is_usb_online(ui) && ui->driver)
...
static int msm72k_set_selfpowered(struct usb_gadget *_gadget, int set)
{
	...
	if (set) {
#ifdef CONFIG_USB_ANDROID_SH_CUST
		atomic_set(&ui->self_powered_req, 1);
#else /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
		atomic_set(&ui->self_powered_req, 0);
#else /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
static int msm72k_is_selfpowered(struct usb_gadget *_gadget)
{
	struct usb_info *ui = container_of(_gadget, struct usb_info, gadget);
	return atomic_read(&ui->self_powered);
}


static int msm72k_is_vbus_connected(struct usb_gadget *_gadget)
{
	struct usb_info *ui = container_of(_gadget, struct usb_info, gadget);
	return is_usb_online(ui);
}

static int msm72k_prevent_pullup(struct usb_gadget *_gadget, int is_prevent)
{
	struct usb_info *ui = container_of(_gadget, struct usb_info, gadget);

	if (atomic_read(&ui->prevent_pullup) == is_prevent)
		return 0;

	atomic_set(&ui->prevent_pullup, is_prevent);

	if (is_prevent) {
		if (is_usb_online(ui))
			msm72k_pullup(_gadget, 0 );
	}

	return 0;
}

int msm72k_set_fullspeed_connect( struct usb_gadget *_gadget, unsigned long enable )
{
	struct usb_info *ui = container_of(_gadget, struct usb_info, gadget);
	...
	ui->fs_connect_enable = enable;
	
	return 0;
}

int msm72k_usb_status_reset( struct usb_gadget *_gadget )
{
	struct usb_info *ui = container_of(_gadget, struct usb_info, gadget);
	unsigned long flags;
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	.is_selfpowered = msm72k_is_selfpowered,
	.is_vbus_connected = msm72k_is_vbus_connected,
	.prevent_pullup = msm72k_prevent_pullup,
	.set_fullspeed = msm72k_set_fullspeed_connect,
	.usb_status_reset = msm72k_usb_status_reset,
	.pullup_internal = msm72k_pullup_internal
#endif /* CONFIG_USB_ANDROID_SH_CUST */
};
					...
					const char *buf, size_t size)
{
	...
	if (!strict_strtoul(buf, 16, &enable)) {
		...
		msm72k_pullup_internal(&the_usb_info->gadget, 0);
		msleep(500);
		msm72k_pullup_internal(&the_usb_info->gadget, 1);
	}
	else
		goto fs_connect_err;
...
fs_connect_err:
	pr_err("%s: usb_full_speed_connect err \n", __func__);
	return -EINVAL;
}
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
static DEVICE_ATTR(fs_connect, 0664, 0, usb_full_speed_connect);
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	retval = device_create_file(&ui->gadget.dev, &dev_attr_fs_connect);
	if (retval != 0)
		dev_err(&ui->pdev->dev,
			"failed to create sysfs entry(fs_connect):"
			"err:(%d)\n", retval);
#endif /* CONFIG_USB_ANDROID_SH_CUST */
...
#ifdef CONFIG_USB_ANDROID_SH_CUST
	device_remove_file(&dev->gadget.dev, &dev_attr_fs_connect);
#endif /* CONFIG_USB_ANDROID_SH_CUST */
