...
static const char shortname[] = "msc_transport";

#define D_MSC_SWITCH_NAME		"usb_mass_storage"
#define D_MSC_FUNCTION_NAME		"usb_mass_storage"
...
static inline struct msc_dev *func_to_dev(struct usb_function *f)
{
...
static int create_bulk_endpoints(struct msc_dev *dev,
...
{
	struct msc_dev *_dev = container_of(w,
					    struct msc_dev,
					    sw_work);

	if( atomic_read( &_dev->online ) )
		msc_control_notify(USB_MSC_NTY_ONLINE_STATE , NULL);
	else
		msc_control_notify(USB_MSC_NTY_OFFLINE_STATE ,NULL);

	switch_set_state(&_dev->sdev, atomic_read( &_dev->online ));
}
...
{
	struct msc_dev  *_dev = container_of(sdev, struct msc_dev, sdev);
	return sprintf(buf, "%s\n", (atomic_read( &_dev->online ) ? "online" : "offline"));
}
...
{
	struct msc_dev	*dev = func_to_dev(f);
	struct usb_request *req;
...
msc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	int			id;
	int			ret;
	struct lun	*curlun = dev->lun; /* MOUNT_FILE */
	...
	/* allocate endpoints */
	ret = create_bulk_endpoints(dev, &msc_fullspeed_in_desc,
	...
	if (curlun) {
		...
		if (ret != 0) {
			printk(KERN_ERR "%s:device_register(lun0) failed\n", __func__);
			goto out;
		}
		ret = device_create_file(&curlun->dev, &dev_attr_file);
		if (ret != 0) {
			printk(KERN_ERR "%s:device_create_file(lun0) failed\n", __func__);
...
void msc_function_exit(void)
{
	if (_msc_dev && _msc_dev->lun) {
		/* MOUNT_FILE */ /* remove lun */
		device_remove_file(&_msc_dev->lun->dev, &dev_attr_file);
		device_unregister(&_msc_dev->lun->dev);
	}

	switch_dev_unregister(&_msc_dev->sdev);
		...
		unsigned intf, unsigned alt)
{
	...
	ret = usb_ep_enable(dev->ep_in,
			ep_choose(cdev->gadget,
				&msc_highspeed_in_desc,
				&msc_fullspeed_in_desc));
	if (ret)
		return ret;
	ret = usb_ep_enable(dev->ep_out,
			ep_choose(cdev->gadget,
				&msc_highspeed_out_desc,
				&msc_fullspeed_out_desc));
	if (ret) {
...
static void msc_function_suspend(struct usb_function *f)
{
	...
	DBG(cdev, "msc suspend\n");
	msc_function_disable(f);
}
...
static int msc_bind_config(struct usb_configuration *c)
{
	...
	/* register switch_dev */
	dev->sdev.name = D_MSC_SWITCH_NAME;
	dev->sdev.print_name = print_switch_name;
	dev->sdev.print_state = print_switch_state;
	ret = switch_dev_register(&dev->sdev);
	if (ret < 0) {
		printk(KERN_ERR "%s:switch_dev_register(%s) failed \n", __func__, dev->sdev.name);
		goto err1;
	}

	INIT_WORK(&dev->sw_work, msc_configure_switch);
...
static struct android_usb_function msc_function = {
	.name = D_MSC_FUNCTION_NAME,
	.bind_config = msc_bind_config,
};

static int __init init(void)
{
	android_register_function(&msc_function);
	return 0;
}
module_init(init);
