...
#define SHDISP_BASE_ADDR 0x8C000000
#define SHDISP_VRAM_BASE 0x8C000800
...
#define SHDISP_IOCTL_CLEAR_SCREEN _IOW(SHDISP_IOC_MAGIC, 24, struct shdisp_main_clear)
#define SHDISP_IOCTL_SET_LCDC_RESET_CFG _IOW(SHDISP_IOC_MAGIC, 25, int)
#define SHDISP_IOCTL_SET_SYSTEM_BUS_SPEED _IOW(SHDISP_IOC_MAGIC, 26, int)
...
static struct pm_qos_request_list *shdisp_system_bus_freq;
static struct pm_qos_request_list *shdisp_cpu_dma_latency;
...
static int shdisp_ioctl_set_lcdc_reset_cfg(void __user *argp);
static int shdisp_ioctl_set_system_bus_speed(void __user *argp);
...
int shdisp_api_main_bkl_on(struct shdisp_main_bkl_ctl *bkl)
{
    ...
    if (bkl->mode == SHDISP_MAIN_BKL_MODE_AUTO_ECO) {
        if (bkl->param != SHDISP_MAIN_BKL_PARAM_2) {
            return SHDISP_RESULT_FAILURE;
        }
    }
...
int shlcdc_api_ir_set_irsel_mode(int irsel_mode)
{
    ...
    if (irsel_mode >= NUM_SHLCDC_IR_IRSD_MODE) {
        return SHDISP_RESULT_FAILURE;
    }
                         ...
                         unsigned int cmd, unsigned long arg)
{
    ...
    case SHDISP_IOCTL_CLEAR_SCREEN:
        ...
        break;
    case SHDISP_IOCTL_SET_LCDC_RESET_CFG:
        ret = shdisp_ioctl_set_lcdc_reset_cfg(argp);
        break;
    case SHDISP_IOCTL_SET_SYSTEM_BUS_SPEED:
        ret = shdisp_ioctl_set_system_bus_speed(argp);
        break;
    default:
        ret = -EFAULT;
        break;
...
static int shdisp_ioctl_get_hw_revision(void __user *argp)
{
	...
	if ( sh_boot_get_hw_revision() ) {
		hw_check = SHDISP_HW_TYPE_1PP;
	} else {
		hw_check = SHDISP_HW_TYPE_1ES;
	}

    ret = copy_to_user(argp, &hw_check, sizeof(int));
...
static int shdisp_ioctl_clear_screen(void __user *argp)
{
    ...
    for (y = 0; y < clear.end_yps - clear.start_yps + 1; y++) {
        for (x = 0; x < clear.end_xps - clear.start_xps + 1; x++) {
            *((volatile unsigned short*)(shdisp_addr + 0x36)) =
                                                                clear.color;
        }
    }
...
static int shdisp_ioctl_set_lcdc_reset_cfg(void __user *argp)
{
    ...
    ret = copy_from_user(&lcdc_reset, argp, sizeof(int));
    ...
    if (lcdc_reset == 1) {
        gpio_tlmm_config(GPIO_CFG(19, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
                                  GPIO_CFG_6MA), GPIO_CFG_ENABLE);
    } else {
        gpio_tlmm_config(GPIO_CFG(19, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
                                  GPIO_CFG_6MA), GPIO_CFG_ENABLE);
    }
...
/*
 * shdisp_ioctl_set_system_bus_speed
 */
...
static int shdisp_ioctl_set_system_bus_speed(void __user *argp)
{
    ...
    ret = copy_from_user(&speed, argp, sizeof(int));
    ...
    if (speed == 1) {
        pm_qos_update_request(shdisp_cpu_dma_latency, 1);
        pm_qos_update_request(shdisp_system_bus_freq, MSM_AXI_MAX_FREQ);
    } else {
        pm_qos_update_request(shdisp_system_bus_freq, PM_QOS_DEFAULT_VALUE);
        pm_qos_update_request(shdisp_cpu_dma_latency, PM_QOS_DEFAULT_VALUE);
    }
...
static int shdisp_ioctl_camera_power(void __user *argp)
{
    ...
    if (OnOff == 1) {
		cam_vreg = vreg_get(NULL, "gp3");
		if( IS_ERR(cam_vreg) ){
			ret = -EFAULT;
			return ret;
		}
		ret = vreg_set_level(cam_vreg, 1200);
		ret = vreg_enable( cam_vreg );
		
		udelay(400);
		
		cam_vreg = vreg_get(NULL, "lvsw1");
		if( IS_ERR(cam_vreg) ){
			ret = -EFAULT;
			return ret;
		}
		ret = vreg_set_level(cam_vreg, 1800);
		ret = vreg_enable( cam_vreg );
...
static int __init shdisp_init(void)
{
    ...
    gpio_tlmm_config(GPIO_CFG(19, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
                              GPIO_CFG_6MA), GPIO_CFG_ENABLE);
    gpio_tlmm_config(GPIO_CFG(23, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
                              GPIO_CFG_6MA), GPIO_CFG_ENABLE);
    gpio_tlmm_config(GPIO_CFG(25, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
                              GPIO_CFG_6MA), GPIO_CFG_ENABLE);
    gpio_tlmm_config(GPIO_CFG(26, 0, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL,
                              GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    gpio_tlmm_config(GPIO_CFG(93, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
                              GPIO_CFG_6MA), GPIO_CFG_ENABLE);
    ...
    shdisp_cpu_dma_latency = pm_qos_add_request(PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
    shdisp_system_bus_freq = pm_qos_add_request(PM_QOS_SYSTEM_BUS_FREQ, PM_QOS_DEFAULT_VALUE);
...
{
    pm_qos_remove_request(shdisp_system_bus_freq);
    pm_qos_remove_request(shdisp_cpu_dma_latency);
    free_irq(SHDISP_INT_IRQ, NULL);
    shdisp_addr = NULL;
    device_destroy(shdisp_class, shdisp_dev);
    class_destroy(shdisp_class);
