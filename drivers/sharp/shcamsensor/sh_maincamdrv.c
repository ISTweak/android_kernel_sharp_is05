...
#define SH_MAINCAM_HCS1_ADDR        0x8c000000
...
static int sh_maincam_drv_read_adjust_data( void __user *argp );
...
static int sh_maincam_drv_read_adjust_data( void __user *argp )
{
    ...
    do {
        param = (struct sh_maincam_ctrl_cmd *)argp;

        sh_smem_common = sh_smem_get_common_address();
        if( sh_smem_common != NULL ) {
            if (copy_to_user((unsigned char *)param->pData,
                   (unsigned char*)sh_smem_common->sh_camOtpData,
                   sizeof(sh_smem_common->sh_camOtpData))) {
                ret = -EFAULT;
                break;
            }
        }
    ...
    do {
        switch (cmd) {
            ...
            case SH_MAINCAM_IOCTL_PRODUCT_REQUEST_ISR:
                ...
                break;
            case SH_MAINCAM_IOCTL_READ_ADJUST_DATA:
                ret  = sh_maincam_drv_read_adjust_data( argp );
                break;
