/* drivers/sharp/shtps/sy3000/tm1601-003/shtps_rmi_spi.c
 *
...
#if defined( CONFIG_SHTPS_SY3000_TM1601_003 )
	#include <sharp/shtps_sy3000_tm1601-003.h>
#elif defined( CONFIG_SHTPS_SY3000_TM1701_001 )
	#include <sharp/shtps_sy3000_tm1701-001.h>
#endif
 ...
 */
	...
	#if defined( CONFIG_SHTPS_SY3000_TM1601_003 )
		#include "shtps_fw_tm1601-003.h"
	#elif defined( CONFIG_SHTPS_SY3000_TM1701_001 )
		#include "shtps_fw_tm1701-001.h"
	#else
...
static int shtps_rmi_write(struct shtps_rmi_spi *ts, u16 addr, u8 data)
{
	...
	_log_msg_send(LOGMSG_ID__SPI_WRITE, "0x%04X|0x%02X", addr, data);
	status = shtps_rmi_spisync(ts, &message);
	_log_msg_recv(LOGMSG_ID__SPI_WRITE_COMP, "%d", status);
	udelay(65);
...
static u8 shtps_rmi_readb(struct shtps_rmi_spi *ts, u16 addr, u8 *buf)
{
	...
	_log_msg_send(LOGMSG_ID__SPI_WRITEADDR, "0x%04X", addr);
	status = spi_sync(ts->spi, &message);
	_log_msg_recv(LOGMSG_ID__SPI_WRITEADDR_COMP, "%d", status);
	if(status){
		return -1;
	}
	spi_transfer_del(&t);
	...
	_log_msg_send(LOGMSG_ID__SPI_READ, "0x%04X", addr);
	status = spi_sync(ts->spi, &message);
	_log_msg_recv(LOGMSG_ID__SPI_READ_COMP, "0x%04X|0x%02X", addr, rx_buf[0]);
	if(status){
		return -1;
	}
	udelay(40);
...
static void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	for(finger = 0;finger < fingerMax;finger++){
		if(((buf[2 + finger / 4] >> (2 * (finger % 4))) & 0x03) == 0x01 ||
		   ((ts->finger_state[finger / 4] >> (2 * (finger % 4))) & 0x03) == 0x01)
		{
			shtps_rmi_read(ts, 
...
#if defined( SHTPS_BLL_ENABLE )
static int shtps_exit_blbootloader(struct shtps_rmi_spi *ts)
{
	int rc;
	
	_log_msg_sync( LOGMSG_ID__BLL_EXIT, "");
	shtps_rmi_write(ts, ts->map.fn01.commandBase, 0x01);
	...
	_log_msg_sync( LOGMSG_ID__BLL_EXIT_DONE, "");
	return 0;
}
#endif /* #if defined( SHTPS_BLL_ENABLE ) */
...
{
	mutex_lock(&shtps_loader_lock);
	if(shtps_flash_writeConfigMain(ts, fwdata) != 0){
		goto err_exit;
	}
...
static int shtps_flash_writeBlConfig(struct shtps_rmi_spi *ts, u8 *fwdata)
{
	...
	if(shtps_exit_blbootloader(ts) != 0){
		goto err_exit;
	}
	mutex_unlock(&shtps_loader_lock);
	return 0;
...
static int shtps_ioctl_reg_read(struct shtps_rmi_spi *ts, unsigned long arg)
{
	...
	if(param.size == 1){
		...
		if(shtps_rmi_read(ts, param.data[0] << 0x08 | param.data[1], &buf, 1)){
			return -EFAULT;
		}
	}
...
static int shtps_ioctl_reg_allread(struct shtps_rmi_spi *ts, unsigned long arg)
{
	...
	if(shtps_rmi_read(ts, param.data[0] << 0x08, dbbuf, 0x100)){
		return -EFAULT;
	}
...
static int shtps_ioctl_reg_write(struct shtps_rmi_spi *ts, unsigned long arg)
{
	...
	if(param.size == 2){
		...
		if(shtps_rmi_write(ts, param.data[0] << 0x08 | param.data[1], param.data[2])){
			return -EFAULT;
		}
	}
	return 0;
...
static int shtps_rmi_open(struct input_dev *dev)
{
	...
	if(shtps_touchpanel_enable(ts) && shtps_fwupdate_enable(ts)){
		...
		if((buf & 0x0F) == 4 || (buf & 0x0F) == 5 || (buf & 0x0F) == 6){
			...
			{
				u8 date[2];
				shtps_rmi_read(ts, 0x0097, &date[0], 1);
				if((date[0] & 0x1F) < 10){
					printk(KERN_DEBUG "[shtps] Device is noncompatible\n");
					return 0;
				}
				shtps_rmi_read(ts, 0x0098, &date[1], 1);
				if((date[0] & 0x1F) <= 10 && (date[1] & 0x0F) < 8){
					printk(KERN_DEBUG "[shtps] Device is noncompatible\n");
					return 0;
				}
			}
			#endif /* #if defined( CONFIG_SHTPS_SY3000_TM1601_003 ) */
