...
static void ShSpiTps_irq_Reset(SpiTpsRec *poSpiTpsRec);
...
#define	SetSDATAHiZ()		ShSpiTps_HsspData(0);	\
							gpio_tlmm_config(GPIO_CFG(SH_TOUCH_HSSP_DATA, 0, GPIO_CFG_INPUT , GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
#define	SetSDATAStrong()	gpio_tlmm_config(GPIO_CFG(SH_TOUCH_HSSP_DATA, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
printk(KERN_DEBUG "[ShSpiTps]copy_to_user Error(ReceiveByte)\n");
	...
	case TPSDEV_GET_SENSOR:
		...
		if(gpoSpiTpsRec->mbIsTestMode)
		{
			...
			for(nCnt = 0; nCnt < 18; nCnt++)
			{
printk(KERN_DEBUG "[ShSpiTps]%2d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d\n", nCnt,  gSensData.bSensData[18*0+nCnt],
																					gSensData.bSensData[18*1+nCnt],
																					gSensData.bSensData[18*2+nCnt],
																					gSensData.bSensData[18*3+nCnt],
																					gSensData.bSensData[18*4+nCnt],
																					gSensData.bSensData[18*5+nCnt],
																					gSensData.bSensData[18*6+nCnt],
																					gSensData.bSensData[18*7+nCnt],
																					gSensData.bSensData[18*8+nCnt],
																					gSensData.bSensData[18*9+nCnt]);
			}
#endif	/* TPS_PRNLOG */
		}
		return nResult;
...
static int ShSpiTps_ReleaseGPIO(SpiTpsRec *poSpiTpsRec)
{
	if(poSpiTpsRec == NULL)
		return -EINVAL;
	/* release the GPIO */
	dev_info(&poSpiTpsRec->mpoSpidev->dev, "releasing gpio pins %d,%d,%d,%d,%d,%d\n",
			 poSpiTpsRec->mnIrqPin, poSpiTpsRec->mnHsspClkPin, poSpiTpsRec->mnHsspDataPin,
			 poSpiTpsRec->mnReset, poSpiTpsRec->mnStandby, poSpiTpsRec->mnVcpin);
...
static void ShSpiTps_irq_Reset(SpiTpsRec *poSpiTpsRec)
{
	int nResult;
#ifdef TPS_PRNLOG
	printk(KERN_DEBUG "[ShSpiTps]ShSpiTps_irq_Reset\n");
#endif	/* TPS_PRNLOG */

	free_irq(poSpiTpsRec->mnIrqPin, poSpiTpsRec);
	/* wakeup set */
	set_irq_type(poSpiTpsRec->mnIrqPin, IRQF_TRIGGER_LOW);
	/* Registering interrupt handler */
	nResult = request_irq(poSpiTpsRec->mnIrqPin, &ShSpiTps_IrqHandler,
	...
	disable_irq(poSpiTpsRec->mnIrqPin);
}
...
printk(KERN_DEBUG "[ShSpiTps]FetchInt\n");
	...
	if(poSpiTpsRec->mbIsTestMode)
	{
		...
		if(0 == ShSpiTps_SpiRead(0x00, gSense, 191))
		{
#ifdef TPS_PRNLOG
...
printk(KERN_DEBUG "[ShSpiTps] X1[%4d] Y1[%4d]\n",(uint16_t)(gSense[7] << 8) + ((uint16_t)gSense[8]),(uint16_t)(gSense[9] << 8) + ((uint16_t)gSense[10]));
			...
			memcpy(&gSensData.bSensData[0], &gSense[11], 180);
	...
	{
		ShSpiTps_irq_Reset(poSpiTpsRec);
		mutex_unlock(&goTpsAccessMutex);
		return;
	}
	/* Reading register */
...
printk(KERN_DEBUG "[ShSpiTps]Set ChargerArmor(%d)\n", poSpiTpsRec->mbIsChargerArmor);
	...
	if(poSpiTpsRec->mbIsFirst == 1)
	{
		/* wakeup set */
		set_irq_type(MSM_GPIO_TO_INT(poSpiTpsRec->mnIrqPin), IRQF_TRIGGER_LOW);
		/* Registering interrupt handler */
...
printk(KERN_DEBUG "[ShSpiTps]ShSpiTps_SensorCheck\n");
	...
	if(0 == ShSpiTps_SpiRead(0x00, rx_buf, 5))
	{
		...
		{
			/* awaiting 50ms and get the first interrupt */
			msleep(50);
...
printk(KERN_DEBUG "[ShSpiTps]%2d %3d %3d %3d %3d %3d %3d %3d %3d %3d %3d\n", nCnt, gSense[11+(18* 0+nCnt)],
						...
						if(0 == ShSpiTps_SpiWriteOne(0x00, bInt, 1))
						{
							...
							for(nCnt = 0; nCnt < 160; nCnt++)
							{
								if(gSense[11+nCnt] < TPS_CALIB_MIN || gSense[11+nCnt] > TPS_CALIB_MAX)
								{
#ifdef TPS_PRNERR
...
printk(KERN_DEBUG "[ShSpiTps]HsspClkCh (%d)\n", nInOut);
	...
	if(nInOut == 0)
	{
		gpio_tlmm_config(GPIO_CFG(SH_TOUCH_HSSP_CLK, 0, GPIO_CFG_INPUT , GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	}
	else
	{
		gpio_tlmm_config(GPIO_CFG(SH_TOUCH_HSSP_CLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	}
    return 0;
}
...
printk(KERN_DEBUG "[ShSpiTps]HsspDataCh (%d)\n", nInOut);
	...
	{
		gpio_tlmm_config(GPIO_CFG(SH_TOUCH_HSSP_DATA, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	}
    return 0;
}
