...
int __init shspamp_init(void)
{
	...
	if(rc < 0){
		printk(KERN_WARNING "ak7811:alloc_chrdev_region error\n");
		return rc;
	}
	...
	if (IS_ERR(shspamp_class)) {
		...
		printk(KERN_WARNING "ak7811:class_create error\n");
	...
	if (IS_ERR(shspamp_device)) {
		...
		printk(KERN_WARNING "ak7811:device_create error\n");
	...
	if(rc < 0){
		printk(KERN_WARNING "ak7811:cdev_add error\n");
		goto err_via_cdev_add;
	}
	
	rc = shspamp_i2c_init();
	if(rc < 0){
		printk(KERN_WARNING "ak7811:shspamp_i2c_init error\n");
		goto err_via_cdev_add;
	}
	
	rc = gpio_request(SHSPAMP_PDN, SHSPAMP_PDN_MSG);
	if(rc < 0){
		printk(KERN_WARNING "ak7811:gpio_request error\n");
	...
	if(rc < 0){
		printk(KERN_WARNING "ak7811:gpio_tlmm_config error\n");
		goto err_via_gpio_config;
	}
#endif
