                                       ...
                                       unsigned len )
{
	...
	switch(req_p->procedure)
	{
		...
		case SHRMTS_WRITE_PROC:
		{
			...
			memcpy(&start_sector, p_sh_smem_common->shrmts_data_buf, 4);
			memcpy(&sector_num, &(p_sh_smem_common->shrmts_data_buf[4]), 4);
			buf = &(p_sh_smem_common->shrmts_data_buf[12]);
	...
	memcpy(&(p_sh_smem_common->shrmts_data_buf[8]), &result, 4);
