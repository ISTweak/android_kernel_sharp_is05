...
static ssize_t
geomagnetic_diag_show(struct device *dev,
        struct device_attribute *attr,
        char *buf)
{
    int busy;
    int16_t t, x=0, y1=0, y2=0;
    int8_t offset[3];
    ...
    if (hwdep_driver.set_enable(1) == 0) {

        yas_cdrv_measure_and_set_offset(offset);
        ...
        if (measure_normal(&busy, &t, &x, &y1, &y2) >= 0) {
            ...
            get_correction_value(&cal, &correct);

            yas_cdrv_set_transformatiom_matrix(YAS_TRANSFORMATION[3]);

            coordinate_conversion(x, y1, y2, t, &data[0], &data[1], &data[2], &correct);
            data[0] /= 1000;
            data[1] /= 1000;
            data[2] /= 1000;
        }
    ...
    return sprintf(buf, "%d,%d,%d", data[0], data[1], data[2]);
}
...
static DEVICE_ATTR(filter_enable, 0664,
        geomagnetic_filter_enable_show, geomagnetic_filter_enable_store);
static DEVICE_ATTR(filter_len, 0664,
        geomagnetic_filter_len_show, geomagnetic_filter_len_store);
static DEVICE_ATTR(filter_threshold, 0664,
        geomagnetic_filter_threshold_show, geomagnetic_filter_threshold_store);
static DEVICE_ATTR(filter_noise, 0664,
        geomagnetic_filter_noise_show, geomagnetic_filter_noise_store);
static DEVICE_ATTR(data, 0444, geomagnetic_data_show, NULL);
static DEVICE_ATTR(status, 0444, geomagnetic_status_show, NULL);
static DEVICE_ATTR(wake, 0220, NULL, geomagnetic_wake_store);
static DEVICE_ATTR(position, 0664,
        geomagnetic_position_show, geomagnetic_position_store);
static DEVICE_ATTR(diag, 0440, geomagnetic_diag_show, NULL);
#if DEBUG
...
void YAS530_SetShape(unsigned char position)
{
    ...
    if(data != NULL)
    {
        if (0 <= position && position <= 1) {
            data->shape = position;
            input_report_abs(data->input_raw, ABS_RAW_SHAPE, position);
        }
    }
}
...
static DEVICE_ATTR(threshold, 0644, geomagnetic_raw_threshold_show, geomagnetic_raw_threshold_store);
static DEVICE_ATTR(distortion, 0644, geomagnetic_raw_distortion_show, geomagnetic_raw_distortion_store);
static DEVICE_ATTR(shape, 0644, geomagnetic_raw_shape_show, geomagnetic_raw_shape_store);
static DEVICE_ATTR(offsets, 0664, geomagnetic_raw_offsets_show, geomagnetic_raw_offsets_store);
...
geomagnetic_work(struct yas_mag_data *magdata)
{
    ...
    if (rt >= 0) {
        ...
        if (rt & YAS_REPORT_DATA) {
            ...
            tmpX = magdata->xyz.v[0];
            tmpY = magdata->xyz.v[1];
            tmpZ = magdata->xyz.v[2];

            if( (data->input_data->abs[ABS_X] != tmpX) ||
                (data->input_data->abs[ABS_Y] != tmpY) ||
                (data->input_data->abs[ABS_Z] != tmpZ) )
            {
                /*  Magnetic Accuracy Init */
                data->input_data->abs[ABS_STATUS] = 99;
            }

            /* report magnetic data in [nT] */
            input_report_abs(data->input_data, ABS_X, tmpX);
            input_report_abs(data->input_data, ABS_Y, tmpY);
            input_report_abs(data->input_data, ABS_Z, tmpZ);
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (hwdep_driver.set_position != NULL) {
        if (hwdep_driver.set_position(CONFIG_SH_YAS530_POSITION) < 0) {
...
{
	gpio_request(117, "gpio_yas530_reset");
