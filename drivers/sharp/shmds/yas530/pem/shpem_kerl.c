/* drivers/sharp/shpem/shpem_kerl.c  (Peripheral Microprocessor Driver)
 *
...
/* Compile Switch */
//#define SHPEM_INIT_SHDN_ENABLE
#define SHPEM_SHTERM_ENABLE
//#define SHPEM_I2C_MUTEX_ENABLE

#ifdef SHPEM_SHTERM_ENABLE
#include <sharp/shterm_k.h>
#endif	/* SHPEM_SHTERM_ENABLE */

#ifdef SHPEM_I2C_MUTEX_ENABLE
#include <sharp/sh_i2c.h>
#endif	/* SHPEM_I2C_MUTEX_ENABLE */
...
/* about GPIOs */
#define SHPEM_GPIO_NUM_FW_CTL	106
#define SHPEM_GPIO_NUM_COMREQ	101
#define SHPEM_GPIO_NUM_COMRDY	85
#define SHPEM_GPIO_NUM_INT		20
#define SHPEM_GPIO_NUM_RESET	16
...
#define SHPEM_GPIO_CNF_FW_CTL	GPIO_CFG(SHPEM_GPIO_NUM_FW_CTL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE
#define SHPEM_GPIO_CNF_COMREQ	GPIO_CFG(SHPEM_GPIO_NUM_COMREQ, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE
#define SHPEM_GPIO_CNF_COMRDY	GPIO_CFG(SHPEM_GPIO_NUM_COMRDY, 0, GPIO_CFG_INPUT , GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE
#define SHPEM_GPIO_CNF_INT		GPIO_CFG(SHPEM_GPIO_NUM_INT   , 0, GPIO_CFG_INPUT , GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE
#define SHPEM_GPIO_CNF_RESET	GPIO_CFG(SHPEM_GPIO_NUM_RESET , 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_10MA), GPIO_CFG_ENABLE
...
static unsigned short shpem_hw_rev = 0x00;
...
static int shpem_not_support_cmd_check(unsigned char cmd_id)
{
	switch(cmd_id)
	{
		case SHPEM_CMD_DSST:
...
static shpem_result_t shpem_com_open(shpem_com_param_t* param)
{
...
#ifdef SHPEM_I2C_MUTEX_ENABLE
	sh_msmi2c_1_mutex_lock(shpem_i2c_client);
#endif	/* SHPEM_I2C_MUTEX_ENABLE */
	...
	{
#ifdef SHPEM_I2C_MUTEX_ENABLE
		sh_msmi2c_1_mutex_free(shpem_i2c_client);
#endif	/* SHPEM_I2C_MUTEX_ENABLE */
		SHPEM_DEBUG_LOG_1("request_irq Error");
		return SHPEM_COM_ERROR;
	}
...
shpem_result_t shpem_com_kerl(shpem_com_param_t* param)
{
	...
	if(shpem_not_support_cmd_check(param->cmd_id) == 0)
	{
		SHPEM_DEBUG_LOG_1("SHPEM_PARAM_ERROR Not Support Command ID = %d", param->cmd_id);
		return SHPEM_PARAM_ERROR;
	}
...
static shpem_result_t shpem_get_hw_rev(unsigned short* rev)
{
...
#ifdef SHPEM_SHTERM_ENABLE
			SHPEM_DEBUG_LOG_2("shterm_k_set_info(1)");
...
static int shpem_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	static unsigned char send_buf[135] = {0};
	static unsigned char recv_buf[129] = {0};
	...
	switch (cmd)
	{
		...
		case SHPEM_IOCTL_RESET:
			...
			ret = SHPEM_SUCCESS;
			break;
		case SHPEM_IOCTL_GET_HWREV:
			SHPEM_DEBUG_LOG_2("SHPEM_IOCTL_GET_HWREV");
			if(copy_to_user(argp, &shpem_hw_rev, sizeof(unsigned short)))
			{
				SHPEM_DEBUG_LOG_1("copy_from_user Error");
				return SHPEM_IO_ERROR;
			}
