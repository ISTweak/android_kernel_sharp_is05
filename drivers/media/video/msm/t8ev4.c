...
#define	T8EV4_VER_QTR_BLK_LINES			38
#define	Q8						0x100
#define	Q10						0x400
...
static uint16_t t8ev4_delay_msecs_stream = 100;
...
{
	static uint16_t max_legal_gain = 0x0200;
	uint8_t gain_msb = 0, gain_lsb = 0;
	uint8_t intg_time_msb = 0, intg_time_lsb = 0;
	uint8_t line_length_pck_msb = 0, line_length_pck_lsb = 0;
	uint16_t line_length_pck, frame_length_lines;
	uint32_t line_length_ratio = 1 * Q12;
	int32_t rc = -1;

	CDBG("t8ev4_write_exp_gain : gain = %d line = %d", gain, line);
	CDBG("t8ev4_write_exp_gain : %d\n",__LINE__);
	if (t8ev4_ctrl->sensormode != SENSOR_SNAPSHOT_MODE) {
		if (t8ev4_ctrl->curr_res == QTR_SIZE) {
			frame_length_lines = T8EV4_QTR_SIZE_HEIGHT +
								 T8EV4_VER_QTR_BLK_LINES;
			line_length_pck = T8EV4_QTR_SIZE_WIDTH +
							  T8EV4_HRZ_QTR_BLK_PIXELS;
			CDBG("t8ev4_write_exp_gain : %d\n",__LINE__);
		} else {
			frame_length_lines = T8EV4_FULL_SIZE_HEIGHT +
								 T8EV4_VER_FULL_BLK_LINES;
			line_length_pck = T8EV4_FULL_SIZE_WIDTH +
							  T8EV4_HRZ_FULL_BLK_PIXELS;
		}
		if (line > (frame_length_lines - T8EV4_OFFSET))
			t8ev4_ctrl->fps = (uint16_t) (30 * Q8 *
										   (frame_length_lines - T8EV4_OFFSET) / line);
		else
			t8ev4_ctrl->fps = (uint16_t) (30 * Q8);
	...
	if (line > (frame_length_lines - T8EV4_OFFSET)) {
		CDBG("t8ev4_write_exp_gain : %d\n",__LINE__);
		line_length_ratio = (line * Q12) / (frame_length_lines -
										   T8EV4_OFFSET);
		line = frame_length_lines - T8EV4_OFFSET;
	} else
		line_length_ratio = 1 * Q12;
	/* range: 0 to 224 */
	...
	line_length_pck = (uint16_t) ((line_length_pck * line_length_ratio) / Q12);
	...
	t8ev4_i2c_write_b_sensor(0x0104,1);/*Grouped parameter hold*/
	...
	rc = t8ev4_i2c_write_b_sensor(REG_LINE_LENGTH_PCK_LO,
								   line_length_pck_lsb);
	if (rc < 0)
		return rc;
	rc = t8ev4_i2c_write_b_sensor(REG_LINE_LENGTH_PCK_HI,
								   line_length_pck_msb);
	if (rc < 0)
		return rc;
	...
	CDBG("gain INE_LENGTH_PCK_HI: %x, INE_LENGTH_PCK_LO: %x\n",line_length_pck_msb, line_length_pck_lsb);
	CDBG("gain INTEGRATION_TIME_HI: %x, RATION_TIME_LO: %x\n",intg_time_msb, intg_time_lsb);
	return rc;
}
...
static int t8ev4_probe_init_done(const struct msm_camera_sensor_info *data)
{
	...
	gpio_direction_input(data->vcm_enable);
...
static int t8ev4_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	rc = gpio_request(data->vcm_enable, "t8ev4");
	printk/*CDBG*/(KERN_INFO "MotorDriver_reset = %d\n", rc);
	if (!rc) {
		gpio_direction_output(data->vcm_enable, 0);
		msleep(20);
		gpio_set_value_cansleep(data->vcm_enable, 1);
		msleep(20);
	} else {
...
//init_pobe_fail:
	...
	init_probe_done:
	printk(" t8ev4_probe_init_sensor finishes\n");
	rc = 0;
	return rc;
}
...
int t8ev4_sensor_open_init(const struct msm_camera_sensor_info *data)
{
	...
	gpio_set_value_cansleep(data->vcm_enable, 0);
...
static int t8ev4_sensor_release(void)
{
	...
	gpio_set_value_cansleep(t8ev4_ctrl->sensordata->vcm_enable, 0);
	msleep(5);
	gpio_direction_input(t8ev4_ctrl->sensordata->vcm_enable);
	gpio_free(t8ev4_ctrl->sensordata->vcm_enable);
							   ...
							   struct msm_sensor_ctrl *s)
{
	...
	if (rc < 0 || t8ev4_client == NULL) {
		printk("*****  failed with not supported error\n");
		rc = -ENOTSUPP;
		goto probe_fail;
	}
	msm_camio_clk_rate_set(T8EV4_DEFAULT_MASTER_CLK_RATE);
	...
	if (rc < 0) {
		printk("probe init sensor faileed \n");
		goto probe_fail;
	}
	...
	probe_fail:
	printk("t8ev4_sensor_probe: SENSOR PROBE FAILS!\n");
...
int t8ev4_check_chipid(void)
{
    int ret = 0;
    int32_t rc;
    unsigned short chipid = 0;

    printk(KERN_INFO "get_t8ev4_chipid \n");

    rc = t8ev4_i2c_read(0x0000, &chipid, sizeof(short));
    if(0 == rc) {
        chipid &= 0xFFF0;
        printk(KERN_INFO "t8ev4 chipid = 0x%04x\n", chipid);
        if(chipid == 0x1400) {
            ret = 1;
        }
    }
...
int t8ev4_reg_read(unsigned short waddr, unsigned short* wData)
{
    int ret = 0;
    int32_t rc;
...
int t8ev4_reg_write(unsigned short waddr, unsigned short wdata)
{
    int ret = 0;
    int32_t rc;
...
int t8ev4_get_otpdata(unsigned char page,
...
printk(KERN_INFO "[%s] len=%d temp=%d in\n", __func__, len, temp);
