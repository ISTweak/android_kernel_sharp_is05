...
/* SH_AUDIO_DRIVER Qualcomm MicMute Patch S */
#include <mach/qdsp5v2/afe.h>
/* SH_AUDIO_DRIVER Qualcomm MicMute Patch E */
...
static int msm_device_rx_mute_info(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 2;
...
static int msm_device_rx_mute_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int msm_device_rx_mute_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int dir = ucontrol->value.integer.value[0];
	int mute = ucontrol->value.integer.value[1];
	return msm_set_device_mute(dir, mute);
}
...
/* SH_AUDIO_DRIVER Qualcomm MicMute Patch E */
#ifdef CONFIG_SH_AUDIO_DRIVER
	MSM_EXT("Device_Rx_Mute", msm_device_rx_mute_info,
			msm_device_rx_mute_get, msm_device_rx_mute_put, 0),
#endif /* CONFIG_SH_AUDIO_DRIVER */
